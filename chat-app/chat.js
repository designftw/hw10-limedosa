...
const app = {
  ...
  computed: {
    populateUsername(actor, username) {},
    messages() {
  ...
  methods: {
    async searchByUsername() {
      this.loading = true;
      const actor = await this.resolver.usernameToActor(this.username);
      this.loading = false;
      if (actor) {
    ...
    lookupUsername(actor) {
      this.resolver.actorToUsername(actor).then((response) => {
        let username = response || "No Username"; // if no username, display this
        let allMessages = document.getElementsByClassName("username");
        if (allMessages) {
          for (let i = 0; i < allMessages.length; i++) {
            if (allMessages[i].innerHTML == actor) {
              allMessages[i].innerHTML = username;
            }
          }
        }
        return username; // Return the resolved username (or "No Username") here
      });
      // this.resolver.actorToUsername(actor).then((response) => {
      //   let username = response || actor; // Use response if it exists, otherwise use actor
      //   let allMessages = document.getElementsByClassName("username");
      //   if (allMessages) {
      //     for (let i = 0; i < allMessages.length; i++) {
      //       if (allMessages[i].innerHTML == actor) {
      ...
      //     }
      //   }
      //   return username; // Return the resolved username (or actor name) here
      // });
      ...
      //       for (let i = 0; i < allMessages.length; i++) {
      //         console.log(i);
      //         console.log(allMessages[i].innerHTML);
      //         if (allMessages[i].innerHTML == actor) {
      //           allMessages[i].innerHTML = response;
      //         }
      //         // allMessages[i].style.color = "purple";
      //         // let thisActorID = allMessages[i].closest(".message .actor").innerHTML;
      //         // allMessages[i].innerHTML = thisActorID;
      //         // console.log(thisActorID);
      //       }
      //     }
      //   }
      // });
